...
Propulsion *Factory::buildPropulsion(Model *parent)
{
    ...
    case 1:
        return new BeardEngine(parent);
    // case 2: return new PistonEngin(parent);
    case 3:
...
Polynomial *Factory::buildPolynomial(char *baseParam)
{
    ...
    if (!ros::param::getCached(parameter, i))
    {
        ROS_FATAL("Invalid parameters for %s in param server!", parameter);
        ros::shutdown();
    }
    ...
    case 0:
    {
        ...
        if (!ros::param::getCached(parameter, list))
        {
            ROS_FATAL("Invalid parameters for %s in param server!", parameter);
            ros::shutdown();
        }
